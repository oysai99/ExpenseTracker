...
# ------------------------ Temporary Test Extractors ------------------------
def extract_amex(pdf_path):
    return [{
        "date": "2025-10-28",
...
def extract_citibank(pdf_path):
    return extract_amex(pdf_path)

def extract_posb(pdf_path):
    return extract_amex(pdf_path)
...
st.title('🏦 Expense Tracker — Test Mode with Sample Transactions')
...
if uploaded_files and st.button('Process and Generate Excel'):
    ...
    for up in uploaded_files:
        ...
        if bank_type == 'Amex':
            ...
            txns = extract_citibank(pdf_path) + extract_amex(pdf_path) + extract_posb(pdf_path)

        for t in txns:
            t['category'] = classify_description(t.get('description'))
            t['base_currency'] = base_curr
            img_path = os.path.join(tmpdir, f"{up.name}_p{t.get('page',0)}.png")
            t['img_path'] = img_path if crop_pdf_line_to_image(pdf_path, t.get('page',0), (0,0,500,50), img_path) else None
            all_txns.append(t)
